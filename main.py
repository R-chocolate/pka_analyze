...
async def analyze_pka(file: UploadFile = File(...)):
    try:
        ...
        # 為了保險，我們只取出 NETWORK 區塊（如果存在），這包含了所有的設備網路邏輯
        ...
        # B. 新世代語義合成 Prompt (全量上下文模式)
        prompt = f"""
        你是一位 Cisco 網路配置專家。我提供了一份 Packet Tracer PKA 檔案的「脫水全量 XML」內容。
        你的任務是通盤解析這份 XML，將所有設定還原為乾淨的 CLI 配置腳本。
        ...
        2. 【Range 合併】：凡是「配置完全相同」的連續埠口，必須合併為 `interface range`。
        3. 【無噪音輸出】：移除所有 '!' 符號、`Building configuration` 等報文。
        4. 【設備分隔】：每個設備使用 ## [HOSTNAME] 作為標題，設備間以 '------' 分隔。
        5. 【純淨腳本】：僅輸出 CLI 指令，嚴禁解釋。

        原始全量脫水數據：
        {context_data}
        """

        # 發送給 Gemini 進行全域解析 (使用最高 Token)
